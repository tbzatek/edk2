  ...
  )
{
  ...
  NET_LIST_FOR_EACH_SAFE (EntryProcessed, NextEntryProcessed, &mNicPrivate->AttemptConfigs) {
    ...
    if (Nid1Valid && Nid2Valid) {
      // Match the NIDs
      if (NvmeOfMatchNid (
            AttemptConfigData->SubsysConfigData.NvmeofSubsysNid,
  ...
  )
{
  ...
  if (!gAttemtsAlreadyRead) {
    ...
    if (EFI_ERROR (Status)) {
      DEBUG ((DEBUG_ERROR, "Reading attempts failed.\n"));
      return EFI_UNSUPPORTED;
    }
  ...
  NET_LIST_FOR_EACH_SAFE (Entry, NextEntry, &mNicPrivate->AttemptConfigs)
  {
    ...
    if (Private == NULL) {
      ...
      if (EFI_ERROR (Status)) {
        DEBUG ((DEBUG_ERROR, "Error while Installing private guid .\n"));
        goto ON_ERROR;
      }
    }
  ...
  )
{
  ...
  DEBUG ((DEBUG_INFO, "NVMEOF_DRIVER_VERSION is %d\n", NVMEOF_DRIVER_VERSION));
  ...
  )
{
  ...
  if (gNvmeOfControllerNameTable != NULL) {
    ...
    if (EFI_ERROR (Status)) {
      DEBUG ((DEBUG_ERROR, "NvmeOfDriverUnload: Error freeing unicode table\n"));
      goto ON_EXIT;
    }
  ...
  if (EFI_ERROR (Status)) {
    DEBUG ((DEBUG_ERROR, "Uninstall multiple protocol for ipv4 failed\n"));
    goto ON_EXIT;
  }
  ...
  )
{
  ...
  if (NvmeOfGetDriverImageName (mImageHandle, &Name) == EFI_SUCCESS) {
  ...
  if (mNicPrivate == NULL) {
    ...
    DEBUG ((DEBUG_ERROR, "NvmeOfDriverEntry: AllocateZeroPool failed\n"));
    goto Error2;
  }
