  ...
  )
{
  ...
  DEBUG ((DEBUG_INFO, "Get Configuration Data from UEFI variable!\n"));
  ...
  if ((AttemptConfigData == NULL) || (AttemptConfigSize == 0)) {
    DEBUG ((DEBUG_ERROR, "AttemptConfigData Read is Blank\n"));
  ...
  )
{
  ...
  if (NvmeOfData->DriverVersion != NVMEOF_DRIVER_VERSION) {
    DEBUG ((
      DEBUG_ERROR,
      "Driver version %d is not matching with Attempt version %d\n",
  ...
  *Contents = NvmeOfGetVariableAndSize (
  ...
  if ((Contents == NULL) || (Size == 0)) {
    DEBUG ((DEBUG_ERROR, "AttemptConfigData Read Failed\n"));
    FreePool (NvmeOfData);
    return EFI_UNSUPPORTED;
  }
